...
describe("deployApp", () => {
  it("creates an app and deployment with pending status initially", async () => {
    ...
    const result = await deployApp(spec, serverId);
  ...
  it("persists app and deployment in the repository", async () => {
    ...
    const storedApp = await repo.getApp(result.app.id);
    expect(storedApp).not.toBeNull();
    expect(storedApp!.name).toBe("persisted-app");

    const storedDeployments = await repo.getDeploymentsForApp(result.app.id);
  ...
  it("transitions deployment through expected states", async () => {
    ...
    const result = await deployApp(validAppSpec(), "server-4");

    expect(result.deployment.status).toBe("healthy");

    const initialDeployments = await repo.getDeploymentsForApp(result.app.id);
